...
- `common/pluginconfig/` — config field registry. Plugins implement the optional `plugin.ConfigRegistrar` interface (`ConfigFields() []pluginconfig.Field`) to declare their config keys with metadata (key, label, type, group, help, sensitive, default). Core collects registrations (framework fields in `bot/core/configfields.go` + all plugins) before `ToViper()`, and the admin panel renders its form dynamically from this registry — adding/removing plugins requires no panel code changes. Called before DI, so implementations must not use injected fields.
