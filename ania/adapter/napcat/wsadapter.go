...
func wsSpreadNotice(n *napcatWebSocketAdapter, noticeType string, data []byte) {
	...
	case "group_upload":
		...
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupUpload != nil {
			n.trigger.OnGroupUpload(notice)
		}
	case "group_admin":
		var notice message.GroupAdminNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupAdmin != nil {
			n.trigger.OnGroupAdmin(notice)
		}
	case "group_decrease":
		var notice message.GroupDecreaseNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupDecrease != nil {
			n.trigger.OnGroupDecrease(notice)
		}
	case "group_increase":
		var notice message.GroupIncreaseNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupIncrease != nil {
			n.trigger.OnGroupIncrease(notice)
		}
	case "group_ban":
		var notice message.GroupBanNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupBan != nil {
			n.trigger.OnGroupBan(notice)
		}
	case "friend_add":
		var notice message.FriendAddNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnFriendAdd != nil {
			n.trigger.OnFriendAdd(notice)
		}
	case "group_recall":
		var notice message.GroupRecallNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupRecall != nil {
			n.trigger.OnGroupRecall(notice)
		}
	case "friend_recall":
		var notice message.FriendRecallNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnFriendRecall != nil {
			n.trigger.OnFriendRecall(notice)
		}
	case "poke":
		var notice message.PokeNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnPoke != nil {
			n.trigger.OnPoke(notice)
		}
	case "lucky_king":
		var notice message.LuckyKingNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnLuckyKing != nil {
			n.trigger.OnLuckyKing(notice)
		}
	case "honor":
		var notice message.HonorNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnHonor != nil {
			n.trigger.OnHonor(notice)
		}
	case "group_msg_emoji_like":
		var notice message.GroupMsgEmojiLikeNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupMsgEmojiLike != nil {
			n.trigger.OnGroupMsgEmojiLike(notice)
		}
	case "essence":
		var notice message.EssenceNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnEssence != nil {
			n.trigger.OnEssence(notice)
		}
	case "group_card":
		var notice message.GroupCardNotice
		if json.Unmarshal(data, &notice) == nil && n.trigger.OnGroupCard != nil {
