...
```json
{
  "groups": { "qq:123456": "你是这个群的管理助手..." },
  "friends": { "qq:7891011": "你是我的私人秘书..." }
}
```
:::
...
| `plugin.ai_chat_bot.query_log.max_result_runes` | `1000` | 工具执行结果写入日志时的截断上限（字符数）；`0` 表示不限制。仅影响日志记录，不影响 AI 实际收到的完整输出；用户输入、工具参数、最终回复始终完整记录 |
