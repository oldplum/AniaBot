...
Web 面板「Query 日志」页记录每次 AI 回复的完整执行过程：触发会话、发送者、用户输入、LLM 轮数、工具调用明细（名称/参数/结果/耗时）、token 用量与最终回复，状态区分执行中 / 成功 / 已停止 / 超时 / 出错，可条件筛选。执行中的记录实时显示已用时，工具调用逐条落盘、随面板轮询即时可见，无需等待执行完成。配置见 [配置详解](/guide/configuration#query-日志-query-log)。

### 计划模式（/plan）

让 AI「先出方案，再动手」。群聊中 @机器人 发送 `/plan on`（私聊直接发送）开启计划模式：AI 只做分析与规划并输出实施计划，修改文件、运行命令、改配置、建任务等副作用操作会被系统自动阻止；确认计划无误后发送 `/plan off` 退出，AI 即可正常执行。`/plan` 无参数可查看当前状态。计划模式按会话隔离，是会话内的临时状态。

### 工具审批（approval）与 bash 三段式权限

默认关闭。开启后，`approval.tools` 列出的工具（默认 `file`）每次执行前都会向会话发送一条确认消息，**消息发送者或机器人管理员**在超时时间（默认 120 秒）内回复「允许」或「拒绝」决定放行与否，超时自动拒绝；子代理 / 定时任务发起的审批仅管理员可批。审批期间正常聊天不受影响。

配置修改类工具（`config_set` / `config_file_set`）**恒需管理员审批**：无论审批开关是否开启，执行前审批提示都会**直接私聊发给机器人管理员**，管理员在私聊中回复「允许/拒绝」才写入（发起会话会收到一条「已通知管理员」提示；管理员在发起会话中回复同样有效；私聊发送失败时提示回退到发起会话）。请求发送者本人不能批准，无权者的审批回复会被拦截并提示，超时自动拒绝。

bash 工具使用独立的命令级三段式权限（无需列入 `approval.tools`）：

1. 命中**黑名单**正则 → 直接拒绝；
2. 命中**白名单**正则 → 直接放行；
3. 两者都不命中（包括两份名单都为空）→ 发起上述人工审批，批准后执行；审批未启用时默认放行（只认黑名单）。

### AI 钩子（hooks）

默认关闭。管理员在面板「扩展配置」页的 `files.hooks_json` 中配置 shell 命令，挂在会话事件上执行：`SessionStart`（会话创建）、`UserPromptSubmit`（用户消息进入，可阻断/注入上下文）、`PreToolUse`（工具执行前，可按工具名正则匹配、可阻断）、`PostToolUse`、`Stop`（回复完成）、`SubagentStop`、`PreCompact`（上下文压缩前）。钩子从 stdin 接收 JSON 载荷；退出码 0 表示通过（stdout 内容注入 AI 上下文）、2 表示阻断（stderr 为原因）、其他退出码仅记日志。保存后数秒内热生效。插件开发者还可实现 Go 钩子接口（`agenthook.Handler`）以代码方式介入同一事件流，见 [AI 引擎（三）](/internals/agent-tools#钩子系统-hooks)。
...
| 群 ID 名单 | 每行一个群 ID（QQ 为 `qq:群号`，其他平台为带前缀的群 ID，如 `fs:oc_xxx`） |
| 用户 ID 名单 | 每行一个用户 ID（QQ 为 `qq:QQ号`，其他平台带前缀），对私聊及群聊消息发送者均生效 |

::: warning 白名单模式注意
`whitelist` 模式下，**放行的群对全体成员开放**（群内屏蔽成员规则除外），无需把成员逐个加进用户 ID 名单；此时用户 ID 名单只作用于私聊。名单留空表示**拦截所有会话**——任何群和私聊都无法触发 AI 回复。
:::

## 白名单管理插件

`pluginwhitelist` · Order = -999（日志插件之后、其余全部插件之前）· 群聊 + 私聊 · 仅管理员

用命令管理**请求拦截插件的那一份名单**，改完立即生效，不需要改配置再 `/reboot`。两个插件共用同一份名单状态，因此面板改配置与 `/wl` 改名单不会互相冲突。
...
群聊中需 **@机器人** 再发命令。ID 可写 `123456`（默认按 QQ 处理）或带平台前缀如 `tg:-100123`，两种写法视为同一个 ID。
...
::: tip 管理员恒放行
管理员的消息永不被本插件拦截，名单配错也能继续发 `/wl` 修回来。系统插件（`/help`、`/reboot`）的 Order 比本插件更靠前，同样不受名单影响。
:::

::: warning 名单落在哪一张表
显式给出 ID 时无法从 ID 本身判断它是群还是人，因此以**命令发出的场景**为准：在群里执行操作群名单，在私聊里执行操作用户名单。想在私聊里把某个群加进名单，请到该群发送 `/wl add`，或直接用面板。
:::
