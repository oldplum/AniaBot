...
- 对话历史持久化保存，重启后继续聊；超过 token 预算 80% 时自动压缩
...
- 记忆**跨会话、跨重启**保留，持久化在 PersistentStorage（`memory:` 命名空间）
...
| 群号名单 | 每行一个群号 |
| QQ号名单 | 每行一个 QQ 号，对私聊及群聊消息发送者均生效 |

::: warning 白名单模式注意
`whitelist` 模式下，**群聊消息必须同时满足「群号在群号名单」且「发送者 QQ 在 QQ 号名单」**才会放行。只填群号名单不会放行该群内其他成员的消息；名单留空表示**拦截所有会话**——任何群和私聊都无法触发 AI 回复。
:::
