...
**AI 自主管理**：AI 在对话中可调用以下工具管理定时任务（默认触发对象为当前会话）：`clock_create`、`clock_list`、`clock_update`、`clock_delete`、`clock_log`。任务触发执行时，还会额外获得 `send_message` 工具，可向**任意群或好友**主动发送消息（文本/图片），不限于任务自身的触发对象——例如一个任务可以"每天早上去群 A 发早报，同时私聊通知管理员"。
