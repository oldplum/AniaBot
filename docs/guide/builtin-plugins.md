...
AI 对话插件内置了一套**动态定时任务**系统，让 AI 和用户都能创建、管理持久化的定时任务。

::: warning 与框架定时任务的区别
本功能与 [框架的 StartCron 定时任务](../plugin/cron.md) 完全独立：框架 cron 是插件在代码里静态注册的（如每日新闻），而 clock 是 AI / 用户**运行时动态创建**、**持久化保存**、**重启不丢失**的，由插件内部独立的调度器管理。
:::

**核心机制**：

- 任务字段包含 **cron 表达式、标题、内容、触发对象（群/好友）、超时时间** 等元信息。
- 到点触发时，相当于自动以 `【定时任务】标题\n 内容` 作为对话内容发给 AI，让 AI 响应。
- 每次触发都是**全新的一次性上下文**，执行完即丢弃，不污染各群/好友的日常对话历史。
- 触发执行时**支持完整工具调用**（搜索、文件、bash、MCP 等），可完成复杂流程。
- 任务持久化到 `PersistentStorage`，重启自动恢复。
...
|------|------|
| `/clock` | 查看当前会话的定时任务 |
| `/clock add <cron> \| <标题> \| <内容>` | 新增任务（触发对象默认为当前会话） |
| `/clock del <id>` | 删除任务 |
| `/clock on <id>` / `/clock off <id>` | 启用 / 停用任务 |
| `/clock info <id>` | 查看任务详情与最近执行记录 |
| `/clock timeout <id> <秒数>` | 设置单次执行超时（0 表示默认） |
| `/clock log [n]` | 查看最近 n 条执行记录（默认 10） |
| `/clock help` | 查看帮助 |
| `/clock list all` / `/clock run <id>` | 管理员：查看全部 / 立即执行一次 |

::: tip add 命令用 `\|` 分隔三段
因为 cron 表达式本身含空格，所以用 `|` 切分：`/clock add 0 8 * * * | 早安 | 大家早上好，新的一天加油！`。AI 也可以在对话中直接说"每天早上 8 点提醒大家打卡"，模型会自动调用工具创建任务。
:::

**AI 自主管理**：AI 在对话中可调用以下工具管理定时任务（默认触发对象为当前会话）：`clock_create`、`clock_list`、`clock_update`、`clock_delete`、`clock_log`。

**触发时的输出处理**：到点触发后，AI 在全新的一次性上下文里执行任务内容（可调用搜索、文件、bash 等工具完成复杂流程）。执行过程中工具主动调用的图片/文件（如 `meme`、`file`）会正常发送到触发对象，但**中间轮的文本不自动回显**（仅记日志）；只有**最终回复**会作为一条消息发送到触发对象。

**执行日志**：每次触发都会记录到持久化日志（触发时间、状态、耗时、token 用量、错误信息），状态包括 `running`/`success`/`timeout`/`error`。任务超时会中止执行并记录 timeout 日志，同时给触发对象发送超时提示。用 `/clock log` 或 `clock_log` 工具查看。
...
plugin:
  ai_chat_bot:
    clock:
      ...
      default_timeout_sec: 120  # 单次触发默认超时（秒）
