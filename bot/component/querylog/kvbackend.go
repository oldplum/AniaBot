...
// （key 为 e:<序号>），写入与单条更新均为 O(1)；序号计数器持久化在 seq 键。
...
func (b *kvBackend) insert(seq uint64, e Entry) {
	...
	if !b.store.Set(ctx, entryKey(seq), e) {
		b.logger.Error("querylog 落盘失败", "id", e.ID)
	}
...
func (b *kvBackend) overwrite(seq uint64, e Entry) {
	if !b.store.Set(context.Background(), entryKey(seq), e) {
		b.logger.Error("querylog 落盘失败", "id", e.ID)
	}
}
...
func (b *kvBackend) listSeqs() []uint64 {
	...
	if err != nil {
		b.logger.Error("querylog 列举键失败", "err", err)
		return nil
	}
