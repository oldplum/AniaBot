...
func (p *MusicPlugin) Start(ctx context.Context, cfg *viper.Viper) error {
	...
	p.Logger.Info("GD音乐插件初始化完成", "baseURL", baseURL)
	return nil
}
...
func (p *MusicPlugin) fetchPage(ctx context.Context, b bot.Bot, sess *searchSession, groupId, userId, msgId message.QID, isGroup bool) bool {
	...
	if err != nil {
		p.Logger.Error("搜索失败", "error", err, "keyword", sess.keyword, "source", sess.source, "count", sess.count, "page", sess.page)
...
func (p *MusicPlugin) sendResults(b bot.Bot, sess *searchSession, groupId, userId, msgId message.QID, isGroup bool) {
	...
	p.Logger.Info("搜索成功", "keyword", sess.keyword, "page", sess.page, "count", sess.count, "results", len(sess.results))
}
...
func (p *MusicPlugin) handleGet(ctx context.Context, b bot.Bot, args []string, groupId, userId, msgId message.QID, isGroup bool) {
	...
	if err != nil {
		p.Logger.Error("获取播放链接失败", "error", err, "url_id", result.URLid, "source", result.Source)
		p.reply(b, isGroup, groupId, userId, msgId, "该歌曲暂无版权或平台不支持下载，换个平台试试（-s netease / -s kuwo）")
		return
	}
	p.Logger.Info("获取链接成功", "url_id", result.URLid, "source", result.Source, "br", songURL.BR)
	...
	p.Logger.Info("发送音乐", "title", title)
}
